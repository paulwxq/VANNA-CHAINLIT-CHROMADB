## bss_car_day_count（`bss_car_day_count` 表用于按日统计进入高速公路服务区的车辆数量）
bss_car_day_count 表`bss_car_day_count` 表用于按日统计进入高速公路服务区的车辆数量，按车辆类型分类，支持车流分析与运营决策。
字段列表：
...
- customer_count (bigint) - 车辆通行数量 [示例: 1114, 295]
